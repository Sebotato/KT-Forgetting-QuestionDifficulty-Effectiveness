...
def attention(self, query, key, value, rel, l1, l2, timestamp, mask=None, dropout=None, device='cpu'):
    ...
    if self.forgetting == False:
        ...
        # Forgetting removed: timestamp information is removed since it is purely used for constructing the forgetting relation coeffients
        if mask is not None:
            scores = scores.masked_fill(mask, -1e9)

        prob_attn = F.softmax(scores, dim=-1)

        # Forgetting removed: Time attention is removed, the problem attention does not have the forgetting component in it
        prob_attn = (1-l2)*prob_attn
