...
class CT_NCM(GDBaseModel):
    ...
    def delay(self, c, c_bar, output, delay, time_lag):
        ...
        if self.decay_function == 'exp':
            c_delay = c_bar + (c - c_bar) * torch.exp(- delay * time_lag.unsqueeze(-1)) # original
        elif self.decay_function == 'rem':
            c_delay = c_bar # remove forgetting
        elif self.decay_function == 'sig':
            sig = nn.Sigmoid()
            c_delay = c_bar + (c - c_bar) * sig(- delay * time_lag.unsqueeze(-1)) # sigmoid decay
        elif self.decay_function == 'pol': 
            c_delay = c_bar + (c - c_bar) * torch.pow(torch.clamp(- delay * time_lag.unsqueeze(-1), min=1e-5, max=1e5), 5)  # poly
        elif self.decay_function == 'log': 
            c_delay = c_bar + (c - c_bar) * torch.log1p(torch.clamp(- delay * time_lag.unsqueeze(-1), min=1e-5, max=1e5))  # log
        elif self.decay_function == 'inv': 
            c_delay = c_bar + (c - c_bar) * (1.0 / (1.0 + torch.abs(- delay * time_lag.unsqueeze(-1)))) # inverse
        else:
            decay_rate = 1.5
            threshold = 0.5
            c_delay = c_bar + (c - c_bar) * torch.where(torch.clamp(- delay * time_lag.unsqueeze(-1), min=0, max=1.0) > threshold, 
                                                        torch.clamp(- delay * time_lag.unsqueeze(-1), min=0, max=1.0), torch.clamp(- delay * time_lag.unsqueeze(-1) * decay_rate, min=0, max=1.0)) # step
